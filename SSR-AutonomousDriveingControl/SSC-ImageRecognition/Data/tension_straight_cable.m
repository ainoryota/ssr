...
for t = 1:1000
    if switching == 0
        if flag == 0    %前輪の分岐開始
            ...
            the_xr = the_x_init; the_yr = 0/180*pi; the_zr = tension_angle;
        ...
        elseif flag == 2
            ...
            if Ln1 <= 250
                ...
                if((t3_+1)*t_step<=1)
                    t3_ = t3_+1;
                    rotm = R("x",E2(1))*R("y",E2(2))*R("x",E2(3))*R("z",-tension_angle*(6*(t3_*t_step)^5-15*(t3_*t_step)^4+10*(t3_*t_step)^3));
                    eulxyz = rotm2eul(rotm,'xyz');
                    the_xf = eulxyz(1); the_yf = eulxyz(2); the_zf = eulxyz(3);
                %後輪のテンションを解く
                elseif ((t3_+1)*t_step >1) && ((t3_+1)*t_step<=2)
                    t3_ = t3_+1;
                    rotm = R("x",E2(1))*R("y",E2(2))*R("x",E2(3))*R("z",-tension_angle);
                    eulxyz = rotm2eul(rotm,'xyz');
                    the_xr = the_x_init; the_yr = 0; the_zr = tension_angle-tension_angle*(6*((t3_)*t_step-1)^5-15*((t3_)*t_step-1)^4+10*((t3_)*t_step-1)^3);
                    the_xf = eulxyz(1); the_yf = eulxyz(2); the_zf = eulxyz(3);
                else
                    the_xr = the_x_init; the_yr = 0; the_zr = 0;
                    the_xf = eulxyz(1); the_yf = eulxyz(2); the_zf = eulxyz(3);
                end
            end
            [Ln2,OrCr,OfCf,OrOf,OrOw1r,OrOw2r,OfOw1f,OfOw2f] = node_cal2(the_xr,the_yr,the_zr,the_xf,the_yf,the_zf,phi_nr1,the_nr,phi_nf1,the_nf,Ln1,Lc,Rw,rw,d,mode);
        ...
        elseif flag == 4
            ...
            if t5 == n_bra+1
               %データの書き出し 
               text = gamma1*180/pi+ "_" + phi_n*180/pi+ "_" + the_nR*180/pi+ "_" + the_nL*180/pi+"_"+mode+"_"+"T";
               T = t1+t2+t3+t4+t5-1;
               data(T+1:500,:)=[];
               if save ==1
...
%111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111%
    ...
    elseif switching ==1
        ...
        %h = acos((d+2*rw)/(2*Rw))*180/pi
        Ln1 = 450;

        if flag == 0
            the_xr = -90/180*pi; the_yr = 0/180*pi; the_zr = 0/180*pi;
            the_xf = -90/180*pi; the_yf = 0/180*pi; the_zf = 0/180*pi; 
            flag = 1;

         elseif flag == 1
            t1 = t1+1; 
            the_xr = -90/180*pi; the_yr = 0/180*pi; the_zr = the_switch*(6*(t1/T1)^5-15*(t1/T1)^4+10*(t1/T1)^3);
            the_xf = -90/180*pi; the_yf = 0/180*pi; the_zf = -the_switch*(6*(t1/T1)^5-15*(t1/T1)^4+10*(t1/T1)^3); 
        ...
        elseif flag == 2
            t2 = t2+1;
            the_xr = (-90+180*(6*(t2/T2)^5-15*(t2/T2)^4+10*(t2/T2)^3))/180*pi; the_yr = 0/180*pi; the_zr = the_switch;
            the_xf = (-90+180*(6*(t2/T2)^5-15*(t2/T2)^4+10*(t2/T2)^3))/180*pi; the_yf = 0/180*pi; the_zf = -the_switch; 
...
%22222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222%
    elseif switching ==2
        ...
        if flag == 0
            the_xr = -90/180*pi; the_yr = 0/180*pi; the_zr = the_switch;
            the_xf = -90/180*pi; the_yf = 0/180*pi; the_zf = -the_switch; 
            flag = 1;
        ...
        elseif flag == 2
            t3 = t3+1;
            the_xr = 90/180*pi; the_yr = 0/180*pi; the_zr = the_switch;
            the_xf = 90/180*pi; the_yf = 0/180*pi; the_zf = -the_switch+ 2*the_switch*(6*(t3/T2)^5-15*(t3/T2)^4+10*(t3/T2)^3); 
        ...
        elseif flag == 3
            t4 = t4+1;
            the_xr = 90/180*pi; the_yr = 0/180*pi; the_zr = the_switch- 2*the_switch*(6*(t4/T2)^5-15*(t4/T2)^4+10*(t4/T2)^3);
            the_xf = 90/180*pi; the_yf = 0/180*pi; the_zf = the_switch; 
...
%333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333%        
    elseif switching ==3
        ...
        if flag == 0
            the_xr = -90/180*pi; the_yr = 0/180*pi; the_zr = 0;
            the_xf = -90/180*pi; the_yf = 0/180*pi; the_zf = 0; 
            flag = 1;
...
%555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555%
    elseif switching ==5
        ...
        elseif flag == 1
            t1 = t1+1; 
            the_xr = -90/180*pi; the_yr = 0/180*pi; the_zr = the_switch;
            the_xf = -90/180*pi; the_yf = 0/180*pi; the_zf = -the_switch+the_switch*(6*(t1/T1)^5-15*(t1/T1)^4+10*(t1/T1)^3); 
...
%666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666%
    elseif switching ==6
        ...
        elseif flag == 1
            t1 = t1+1; 
            the_xr = -90/180*pi; the_yr = 0/180*pi; the_zr = the_switch-the_switch*(6*(t1/T1)^5-15*(t1/T1)^4+10*(t1/T1)^3);
            the_xf = -90/180*pi; the_yf = 0/180*pi; the_zf = -the_switch; 
...
%1010101001010101010101010101010101010101010101010101010101010101010101010101010101010101010101010101010101010101010101010101
    elseif switching ==10
        omega_r = 0;
        omega_f = 0;
        %メインケーブルに対して，どのケーブルに乗っているのかの情報，
...
%nnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnn%
        ...
        elseif flag == 0
            ...
            the_xr = the_x_init; the_yr = 0/180*pi; the_zr = 0/180*pi;
            the_xf = the_x_init; the_yf = 0/180*pi; the_zf = 0/180*pi;
            
            Ln1 = 450 - omega_c*t0*t_step*pi/180*r_v;
            [Ln2,OrCr,OfCf,OrOf,OrOw1r,OrOw2r,OfOw1f,OfOw2f] = node_cal2(the_xr,the_yr,the_zr,the_xf,the_yf,the_zf,phi_nr1,the_nr,phi_nf1,the_nf,Ln1,Lc,Rw,rw,d,mode);
...
%             [Ln1,OrCr,OfCf,OrOf,OrOw1r,OrOw2r,OfOw1f,OfOw2f] = node_cal1(the_xr,the_yr,the_zr,the_xf,the_yf,the_zf,phi_nr1,the_nr,phi_nf1,the_nf,Ln2,Lc,Rw,rw,d,mode);
            ...
            if Ln1 < 340 + omega_c*t_step*pi/180*r_v
                flag = 10;
                t0 = 0;
              %定速走行カウント
            end
        elseif flag == 5
            ...
            Ln1 = -40 - omega_c*t0*t_step*pi/180*r_v;
            [Ln2,OrCr,OfCf,OrOf,OrOw1r,OrOw2r,OfOw1f,OfOw2f] = node_cal2(the_xr,the_yr,the_zr,the_xf,the_yf,the_zf,phi_nr1,the_nr,phi_nf1,the_nf,Ln1,Lc,Rw,rw,d,mode);
