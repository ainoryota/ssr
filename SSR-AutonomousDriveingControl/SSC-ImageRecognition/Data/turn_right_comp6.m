...
%メインケーブルに対して，どのケーブルに乗っているのかの情報，
 ...
 [Ln2,OrCr,OfCf,OrOf,OrOw1r,OrOw2r,OfOw1f,OfOw2f] = node_cal2(the_yr,the_xr,the_zr,the_yf,the_xf,the_zf,0,0,Ln1,Lc,Rw,rw,d,mode,0,0);
        ...
        %% 各関節角度の計算
            ...
            %収束計算
            fun = @(x)Joint_angle_calc6(x,the_yr,the_xr,the_zr,the_yf,the_xf,the_zf,phi_nr1,phi_nr2,the_nr,phi_nf1,phi_nf2,the_nf,...
            ...
            = Joint_angle_calc6(x,the_yr,the_xr,the_zr,the_yf,the_xf,the_zf,phi_nr1,phi_nr2,the_nr,phi_nf1,phi_nf2,the_nf,...
                                gamma1,a1,a2,a3,Lc,OrCr,OfCf,OrOf,g0,g2,g3,g4,g6,w0,w2,w3,w4,w6,mode);
...
function [Ln2,OrCr,OfCf,OrOf,OrOw1r,OrOw2r,OfOw1f,OfOw2f] = node_cal2(the_yr,the_xr,the_zr,the_yf,the_xf,the_zf,phi_n,the_n,Ln1,Lc,Rw,rw,d,mode,gap_mode_r,gap_mode_l)
    ...
    elseif mode == 2 %左分岐モード
        ...
        OrOw1r = [2*Rw*sin(gap_mode_r);0;0] + R("y",the_yr)*R("x",the_xr)*[0;d/2+rw;0];
        OrOw2r = [2*Rw*sin(gap_mode_r);0;0] + R("y",the_yr)*R("x",the_xr)*[2*Rw*sin(the_zr);d/2+rw-2*Rw*cos(the_zr);0];
        %前輪
        OfCf =   [2*Rw*sin(gap_mode_l);0;0] + R("y",the_yf)*R("x",the_xf)*[Rw*sin(the_zf);d/2+rw-Rw*cos(the_zf);0];
        OfOw1f = [2*Rw*sin(gap_mode_l);0;0] + R("y",the_yf)*R("x",the_xf)*[0;d/2+rw;0];
        OfOw2f = [2*Rw*sin(gap_mode_l);0;0] + R("y",the_yf)*R("x",the_xf)*[2*Rw*sin(the_zf);d/2+rw-2*Rw*cos(the_zf);0];
    else
    end
