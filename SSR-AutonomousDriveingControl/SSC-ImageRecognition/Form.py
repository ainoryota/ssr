...
class Form(object):
    def __init__(self,robot):
        ...
        #チェックボックスの作成
        self.v1 = tk.BooleanVar()#前進
        self.v2 = tk.BooleanVar()#後退
        self.v3 = tk.BooleanVar()#右分岐
        self.v4 = tk.BooleanVar()#左分岐
        self.v5 = tk.BooleanVar()#auto分岐
        self.v6 = tk.BooleanVar()#前輪T
        ...
        self.rs=RealSense(self.root,self.br);
        self.root.mainloop()
    ...
    def change(self):           
        ...
        if self.v3.get() == True:
            if (self.v5.get() == False) and (self.v6.get() == False):
                self.br.FileBranch("normal_switching.csv",self.v3.get(),self.v8.get())
                self.v3.set(False) 
                self.v4.set(True) 
            elif (self.v5.get() == True) and (self.v6.get() == True):
                self.br.FileBranch("tension_switching.csv",self.v3.get(),self.v8.get())
                self.v3.set(False) 
                self.v4.set(True)
            else:
        ...
        if self.v3.get() == True:                                     #通常どおりにプログラムを流すか(右分岐モードのとき)
            if self.v5.get() == False:                                    #前輪にテンションがかかっていないとき
                if self.v6.get() == False:                                    #後輪にテンションがかかっていないとき
                    self.br.FileBranch("f_tension_r-n.csv",self.v3.get(),False)
                else:                                                    #後輪にテンションがかかっているとき
                    self.br.FileBranch("f_tension_r-t.csv",self.v3.get(),False)
                self.v5.set(True)
            else:                                                    #前輪にテンションがかかっているとき
                if self.v6.get() == False:                                    #後輪にテンションがかかっていないとき
                    self.br.FileBranch("f_normal_r-n.csv",self.v3.get(),False)
                else:                                                    #後輪にテンションがかかっているとき
                    self.br.FileBranch("f_normal_r-t.csv",self.v3.get(),False)
                self.v5.set(False)     
        else:                                                    #逆向きにプログラムを流すか(左分岐モードのとき)
            if self.v5.get() == False:                                    #前輪にテンションがかかっていないとき
                if self.v6.get() == False:                                    #後輪にテンションがかかっていないとき
                    self.br.FileBranch("r_tension_f-n.csv",self.v3.get(),False)
                else:                                                    #後輪にテンションがかかっているとき
                    self.br.FileBranch("r_tension_f-t.csv",self.v3.get(),False)
                self.v5.set(True)
            else:                                                    #前輪にテンションがかかっているとき
                if self.v6.get() == False:                                    #後輪にテンションがかかっていないとき
                    self.br.FileBranch("r_normal_f-n.csv",self.v3.get(),False)
                else:                                                    #後輪にテンションがかかっているとき
                    self.br.FileBranch("r_normal_f-t.csv",self.v3.get(),False)
                self.v5.set(False)     
    ...
    def branch(self,num):
        ...
        print(csv_name,num)
        self.br.FileBranch(csv_name,self.v1.get(),self.v8.get())
