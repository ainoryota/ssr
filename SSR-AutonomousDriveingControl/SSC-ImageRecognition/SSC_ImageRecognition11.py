...
def IR(color_image,depth_image,ir_image,robot_rotation,extMode=True):
    ...
    if(extMode):
        ...
        #OutputController().msgPrint("minMax",minColor,maxColor)
        #minColor*=0.9
        #maxColor*=1.1
        for seed in seeds:
            x = seed[0]
        ...
        while len(seeds):
            ...
            for direct in directs:
                ...
                if cur_x < 0 or cur_y < 0 or cur_x >= width or cur_y >= height:
                    continue
                if (frag[cur_y][cur_x][0]>0) and diff(ir_image[cur_y][cur_x][0] ,ir_image[y][x][0]) <= 5 :
                    frag_count+=1
            #OutputController().msgPrint(frag_count)
            if(frag_count<3):
                continue;
            ...
            for direct in directs:
                ...
                #OutputController().msgPrint(ir_image[cur_y][cur_x][0], ir_image[y][x][0],diff(ir_image[cur_y][cur_x][0] , ir_image[y][x][0]))
