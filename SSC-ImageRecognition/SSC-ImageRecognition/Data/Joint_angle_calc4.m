...
function [F,the_2r,the_3r,the_1r,the_2f,the_3f,the_1f,the_03x] ...
   = Joint_angle_calc4(x,the_yr,the_xr,the_zr,the_yf,the_xf,the_zf,phi_nr1,phi_nr2,the_nr,phi_nf1,phi_nf2,the_nf,...
    ...
    Co_0r = Rod_R(Co_0r(1,3),Co_0r(2,3),Co_0r(3,3),pi)*Co_0r;       %可動範囲を考慮した座標系の設定
    ...
    the_03x = atan2(-M_0r(2,3),M_0r(2,2));
    ...
    m3 = cross((OrCr + R("y",the_yr)*R("x",the_xr)*R("z",the_zr)*R("z",-the_3r+pi/2)*R("x",a2)*R("z",-the_2r)*R("x",-a1)*R("z",-the_1r)*R("z",-pi)*R("y",a3)*g3),[-w3*sin(gamma1);0;w3*cos(gamma1)]);
    m4 = cross((OrCf + R("y",the_yr)*R("x",the_xr)*R("z",the_zr)*R("z",-the_3r+pi/2)*R("x",a2)*R("z",-the_2r)*R("x",-a1)*R("z",-the_1r)*R("z",-pi)*R("y",2*a3)*R("z",the_1f)*R("x",a1)*g4),[-w4*sin(gamma1);0;w4*cos(gamma1)]);
    m6 = cross((OrCf + R("y",the_yr)*R("x",the_xr)*R("z",the_zr)*R("z",-the_3r+pi/2)*R("x",a2)*R("z",-the_2r)*R("x",-a1)*R("z",-the_1r)*R("z",-pi)*R("y",2*a3)*R("z",the_1f)*R("x",a1)*R("z",the_2f)*R("x",-a2)*R("z",the_3f + pi/2)*g6),[-w6*sin(gamma1);0;w6*cos(gamma1)]);
