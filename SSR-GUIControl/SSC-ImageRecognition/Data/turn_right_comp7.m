...
%   t_step = 0.0485; %50[ms] キリが悪いと挙動がおかしくなる．48msでだめ
    ...
    [Y_12,X_12,Z_12] = slerp_yxz_euler(quat1,quat2,fix(n_bra/2));            %偶数じゃなかったら．．．
    Y = [Y_01;Y_12];
    X = [X_01;X_12];   %連結
    Z = [Z_01;Z_12];
    
 
    [L_def,OrCr,OfCf,OrOf,OrOw1r,OrOw2r,OfOw1f,OfOw2f] =  node_cal1(0,0,0,Y(n_bra),X(n_bra),Z(n_bra),phi_n,0,0,Lc,Rw,rw,d,mode); %前輪分岐直後のLn1の長さ（これから後輪が移動するべき距離，空間だとLcと異なる）
